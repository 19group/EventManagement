...
<section class="payment_gateway_options"  id="gateway_{{config('attendize.payment_gateway_pesapal')}}">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                {!! Form::label('pesapal[customerKey]', 'Cutomer Key', array('class'=>'control-label ')) !!}
                {!! Form::text('pesapal[customerKey]', $account->getGatewayConfigVal(config('attendize.payment_gateway_pesapal'), 'customerKey'),[ 'class'=>'form-control'])  !!}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {!! Form::label('pesapal[secretKey]', 'Secret Key', ['class'=>'control-label ']) !!}
                {!! Form::text('pesapal[secretKey]', $account->getGatewayConfigVal(config('attendize.payment_gateway_pesapal'), 'secretKey'),[ 'class'=>'form-control'])  !!}
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="form-group">
                {!! Form::label('pesapal[callBack]', 'Call back url', array('class'=>'control-label ')) !!}
                {!! Form::text('pesapal[callBack]', $account->getGatewayConfigVal(config('attendize.payment_gateway_pesapal'), 'callBack'),[ 'class'=>'form-control'])  !!}
            </div>
        </div>
    </div>
