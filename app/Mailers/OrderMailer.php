...
class OrderMailer
{
    ...
    public function sendOrderInvitationLetters($order)
    {

        Log::info("Sending invitation letter to: " . $order->email);

        $data = [
            'order' => $order,
        ];

        Mail::send('Mailers.TicketMailer.SendOrderInvitationLetters', $data, function ($message) use ($order) {
            $message->to($order->email);
            $message->subject('Your invitation letters for the event ' . $order->event->title);
